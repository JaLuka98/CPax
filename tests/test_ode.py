...
def test_HO_Newtonian():
    ...
    assert jnp.allclose(newtonian_1d_HO(ps0, t0), jnp.array([0.0, -1.0]), atol=1e-7), "Incorrect dynamics for initial state."
...
def test_HO_Hamiltonian():
    ...
    assert jnp.allclose(hamiltonian_1d_HO(ps0, t0), jnp.array([0.0, -1.0]), atol=1e-7), "Incorrect dynamics for initial state."
